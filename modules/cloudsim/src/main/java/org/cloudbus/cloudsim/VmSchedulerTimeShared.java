...
public class VmSchedulerTimeShared extends VmScheduler {
	...
	protected boolean allocatePesForVm(String vmUid, List<Double> mipsShareRequested) {
		...
		//System.out.println("[DEBUG]: this.pesInUse" + this.pesInUse);
		...
		for (Double mipsRequested : mipsShareRequested) {
			//List<String> vmList = getVmsMigratingOut();
			//List<String> vmListIn = getVmsMigratingIn();
			//boolean b1 = getVmsMigratingOut().contains(vmUid);
			//boolean b2 = getVmsMigratingIn().contains(vmUid);
