...
public class Channel {
	...
	private double reseveredBandwidth; //[NEW] reserved Bandwidth for channel to recover the transmission
...
//	private SDNVm dstVm;
	...
	public Channel(int chId, int srcId, int dstId, List<Node> nodes, List<Link> links, double bandwidth, SDNVm srcVm, SDNVm dstVm) {
		...
		this.srcVm = srcVm;
//		this.dstVm = dstVm;
		packetScheduler.setTimeOut(Configuration.PACKET_TIME_OUT); // If the packet is not successfull in 3 seconds, it will be discarded.
	}
	...
	public boolean adjustDedicatedBandwidthAlongLink() {
		if(chId == -1) 
			return false;
		//TODO text for migration channel
		//if(chId >= 10000)
		//	System.out.println("Migration BW Sharing test!!");
	...
	*/
	...
	public double getReseveredBandwidth() {
		return reseveredBandwidth;
	}

	public void setReseveredBandwidth(double reseveredBandwidth) {
		this.reseveredBandwidth = reseveredBandwidth;
	}
	//[NEW]
	...
	//[NEW] check two channels sharing the same link or not
	public boolean isSharingLink(Channel ch) {
		//Set<Node> nodeSet = new HashSet<Node>(ch.getNodeList());
		Set<Link> linkSet = new HashSet<Link>(ch.getLinkList());
		Set<Link> linksLocal = new HashSet<Link>(this.links);
		Set<Link> test = linkSet;
		test.removeAll(linksLocal);
		if(test.containsAll(linkSet))
			return false;
		else
			return true;
	}
