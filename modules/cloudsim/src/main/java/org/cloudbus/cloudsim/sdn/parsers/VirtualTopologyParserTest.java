...
public class VirtualTopologyParserTest {
	//change all default links to non default links and increase the bandwidth of virtual links
	private static int flowNumbers=0;
	
	private double bw_multipler = Configuration.FLOW_MULTIPLER;
	...
	public VirtualTopologyParserTest(String topologyFileName, int userId) {
		this.vmsFileName = topologyFileName;
		this.userId = userId;
		bw_multipler = Configuration.FLOW_MULTIPLER;
		parse();
	}
	...
	private Hashtable<String, Integer> parseVMs(JSONObject doc) {
		...
		while(iter.hasNext()){
			...
			//[NEW] dirty_rate and mig deadline
			double dirty_rate_factor = (Double) node.get("dirty_rate");
			double deadline = (Double) node.get("mig_deadline");
			...
			for(int n=0; n<nums; n++) {
				...
				if(nodeType.equalsIgnoreCase("vm")){
					...
					vm.setDirtyFactor(dirty_rate_factor);
					vm.setDirtyRate(dirty_rate_factor * vm.getRam() * 8); //Gbit/s
					...
					sf.setDeadline(deadline);
					sf.setDirtyRate(dirty_rate_factor);
					sf.setDirtyRate(dirty_rate_factor * sf.getRam() * 8); //Gbits
	...
	private Hashtable<String, Integer> parseLinks(JSONObject doc, Hashtable<String, Integer> vmNameIdTable) {
		...
		while(linksIter.hasNext()){
			...
			if(reqLat != null)
				lat = (Double) reqLat;
			if(reqBw != null)
				bw = (long) ((Long) reqBw*this.bw_multipler);
			...
			if(name == null || "default".equalsIgnoreCase(name)) {
				...
				//flowId = -1;
				flowId = flowNumbers++;
				name = "default-"+src+dst;
				bw = (long) ((long) 2000000L * bw_multipler); //2Mbps for default-vm-vm link
			}
			else {
				flowId = flowNumbers++;
			}
