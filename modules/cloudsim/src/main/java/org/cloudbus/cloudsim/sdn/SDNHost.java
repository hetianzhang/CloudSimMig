...
//			System.err.println(CloudSim.clock()+":"+vm + " is reallocated: "+ mipsAllocatedAfter);
	...
	//[NEW] return the complete route of node list
	public List<Node> getCompleteVmRoute(int src, int dest, int flowId){
		List<Node> route = new ArrayList<>();
		route.add(this); // add srcHost first
		Node nextnode = this.getVMRoute(src, dest, flowId);
		while(nextnode!=null) {
			route.add(nextnode);
			nextnode = nextnode.getVMRoute(src, dest, flowId);
		}
		return route;
	}
