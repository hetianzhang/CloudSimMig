...
public abstract class VmMigrationPolicy {
	...
	public List<Map<String, Object>> getMigrationMap(List<SDNHost> hosts) {
		//TODO remove all fake vm for migration
		Map<Vm, Host> vmToHost = buildMigrationMap(hosts);
		
		// Make a list from the migration map0
	...
	protected Host moveVmToHost(SDNVm vmToMigrate, List<Host> targetHosts) {		
		...
		for(int i=0; i<targetHosts.size(); i++) {
			...
			if (result) { // get the first host that is suitable for the VM migration
	...
	protected List<SDNHost> getOverutilizedHosts(List<SDNHost> hosts) {
		...
		for(SDNHost host:hosts) {
			...
			// 2. Each VMs in the Host are also overloaded
			
			//TODO add every host whether it is overloaded or not.
	...
	protected List<SDNVm> getMostUtilizedVms(List<SDNHost> hosts) {
		...
		if(overHosts != null && overHosts.size() != 0) {
			for(SDNHost host: overHosts) {
				...
				if(mostUtilized != null && mostUtilized.getRam() != 0)
...
//			}
		...
		//TODO change the over-utilized threshold for host
		double overloadPercentile = host.getMonitoringValuesOverloadMonitor().getOverUtilizedPercentile(startTime, endTime, 0.6);
