...
public class ServiceFunctionAutoScaler {
	// This class defines SFC scale policy
	...
	public void scaleSFC() {
		...
		Set<ServiceFunction> vmsToScaleUp = new HashSet<ServiceFunction>();
		...
		for(ServiceFunctionChainPolicy policy:sfForwarder.getAllPolicies()) {
			...
			if(endToEndDelay > policy.getDelayThresholdMax()) {
				...
				// Scale BW
				if(Configuration.SFC_AUTOSCALE_ENABLE_BW) {
				...
				if(Configuration.SFC_AUTOSCALE_ENABLE_VM) {
					...
					for(int sfId:sfc) {
						ServiceFunction sf = (ServiceFunction)nos.findVm(sfId);
						if(isVmOverloaded(sf)) {
							vmsToScaleUp.add(sf);
						}
					}
				}
		...
		for(ServiceFunction sfDown:vmsToScaleDown) {
			if(vmsToScaleUp.contains(sfDown)) {
		...
		for(ServiceFunction sf:vmsToScaleUp) {
	...
	private void vmScaleUp(ServiceFunction sf) {
		if(Configuration.SFC_AUTOSCALE_ENABLE_VM_VERTICAL && increaseVmCapacity(sf)) {
			...
			ServiceFunction newSf = duplicateSF(sf);
	...
	private void vmScaleDown(ServiceFunction sf) {
		...
		if(newMips != null) {
			...
			for(ServiceFunction subSf:allSf) {
				nos.updateVmMips(subSf, newMips.numberOfPes, newMips.mips);
			}
		}
		else {
	...
	private ServiceFunction duplicateSF(ServiceFunction orgSf) {
	...
	*/
	...
	private CPUPeMips getVmMipsStepUp(ServiceFunction sf) {
		double orgMips = sf.getMips();
		double newMips = orgMips * 1;//2; // does not change
	...
	protected void printMonitoredValue() {
		for(ServiceFunctionChainPolicy policy:this.sfForwarder.getAllPolicies()) {
			...
			for(int i=0; i < vmIds.size()-1; i++) {
				...
				if(i != 0) {
					SDNVm vm = (SDNVm)nos.findVm(fromId);  
