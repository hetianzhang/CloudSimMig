 ...
 */
	...
	public static void printGroupMigrationStatistics() {
		Log.printLine("========== GROUP MIGRATIONS STATISTIC ===========");
		
	}
	
	public static void printMigrationPlan(Map<Integer, LinkedList<Memcopying>> migPlan) {
		Log.printLine("========== MIGRATIONS PLAN ===========");
		for(Entry<Integer, LinkedList<Memcopying>> migGroup: migPlan.entrySet()) {
			Log.printLine("Group: "+migGroup.getKey());
			for(Memcopying mig: migGroup.getValue()) {
				Log.print("VM "+mig.getMigVm() +": "+mig.getSrcHost()+"->"+mig.getDstHost()+"|"+mig.getChId()+" ");
			}
			Log.print("\n");
		}
	}
	
	public static void printMigration(Memcopying mig) {
		Log.printLine("========== MIGRATIONS STATISTIC===========");
		Log.printLine("Migration VM #"+mig.getMigVm() +": Host #"+mig.getSrcHost()+"-> Host #"+mig.getDstHost()+"|"+mig.getChId()+
				" Avg. BW: "+mig.getAmountMemCopied()/mig.getMigTime()+" Estimated: "+ mig.getEstimateMigTime()+" Real:"+mig.getMigTime());
		Log.printLine("RAM:"+mig.getVmRAM()+" dt thres:"+ mig.getDowntimeThres() + " max iteration:"+ mig.getIterationThres());
		Log.print(" start: "+ mig.getStartTime());
		Log.print(" finish: "+mig.getFinishTime());
		Log.print(" mt: "+ mig.getMigTime());
		Log.print(" dt: "+ mig.getDowntime());
		Log.print(" iter: " + mig.getIterationNum());
		Log.print(" Deadline: "+mig.getDeadline());
		//Log.print(" priority: "+ mig.getPriority());
		Log.printLine();
	}
	
	public static void printMigrationCSV(Memcopying mig) {
		//"id,src,dst,arrival,deadline,start,finish,migtime,downtime,bw,iter,slack"
		Log.printLine(mig.getMigVm()+","+mig.getSrcHost()+","+mig.getDstHost()+","+mig.migrate.get("time")+","+mig.migrate.get("deadline")
		+","+mig.getStartTime()+","+mig.getFinishTime()+","+mig.getMigTime()+","+mig.getDowntime()+","+(mig.getAmountMemCopied()/mig.getMigTime())
		+","+mig.getIterationNum()+","+((double)mig.migrate.get("time")+mig.getDeadline()-mig.getFinishTime()));
	}
	
	public static void printMigrationSummaryCSV(List<Memcopying> mcList) {
		Log.printLine("id,src,dst,arrival,deadline,start,finish,migtime,downtime,bw,iter,slack");
		for(Memcopying mc:mcList) {
			LogPrinter.printMigrationCSV(mc);
		}
	}
	
	public static void printMigrationSummary(List<Memcopying> mcList) {
		Log.printLine("========== MIGRATIONS SUMMARY===========");
		double downtimeSum = 0, migtimeSum = 0;	
		double multiStart = Double.MAX_VALUE, multiFinish = Double.MIN_VALUE;
		long dataSum = 0;
		
		for(Memcopying mc:mcList) {
			LogPrinter.printMigration(mc);
			long dataSize = mc.getAmountMemCopied();
			double downtime = mc.getDowntime();
			if(mc.getFinishTime()>multiFinish) {
				//multiFinish = mc.getFinishTime();
				multiFinish = mc.getPostMigEnd();
			}
			if(mc.getStartTime()<multiStart) {
				//multiStart = mc.getStartTime();
				multiStart = mc.getPreMigStart();
			}
			//double migTime = mc.getFinishTime() - mc.getStartTime(); network time
			double migTime = mc.getPostMigEnd() - mc.getPreMigStart();
			downtimeSum = downtimeSum + downtime;
			migtimeSum = migtimeSum + migTime;
			dataSum += dataSize;
		}
		if(mcList.size()!=0) {
		Log.printLine("Average Migration Time:"+ (migtimeSum / mcList.size()));
		Log.printLine("Average Downtime:"+ (downtimeSum / mcList.size()));
		Log.printLine("Avergae Transferred Data:"+ (dataSum / mcList.size()));
		Log.printLine("Total Multiple Migration Time:"+ (multiFinish-multiStart));
		Log.printLine("Sum of Migration Time:"+ migtimeSum);
		Log.printLine("Total Downtime:"+ downtimeSum);
		Log.printLine("Total Transferred Data:"+ dataSum);
		}
	}
	...
	public static void printAlgRunTime(String alg, double algTime) {
		Log.printLine("Algorithm: "+alg);
		Log.printLine("runtime (millisecond): "+algTime);
	}
