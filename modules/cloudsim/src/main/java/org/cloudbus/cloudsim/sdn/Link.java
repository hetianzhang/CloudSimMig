...
public class Link {
	...
	*/
	...
	//[NEW]: FIX THE BUG: when one channel Bw is 0. Bw sharing calculation is wrong.
	private int getZeroDedicatedChannelCount(Node from) {
		int num = 0;
		for(Channel ch: getChannels(from)) {
			if(ch.getChId() != -1 && ch.getAllocatedBandwidth() == 0) {
				num ++;
			}
		}
		return num;
	...
	//[NEW]
	private int getZeroSharedChannelCount(Node from) {
		int num = 0;
		for(Channel ch: this.getChannels(from)) {
			if(ch.getChId() == -1 && ch.getAllocatedBandwidth() == 0) {
				num++;
			}
		}
