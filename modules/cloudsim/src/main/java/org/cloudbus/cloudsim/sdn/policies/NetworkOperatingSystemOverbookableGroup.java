...
public class NetworkOperatingSystemOverbookableGroup extends NetworkOperatingSystemOverbookable {
	...
	protected List<VmGroup> createVmGroup(Collection<Vm> vms, Collection<Arc> links) {
		...
		List<Vm> vnfPool = new ArrayList<Vm>();
		...
		//remove the middlebox (VNF) from vmPool and create VNF pool
		List<Vm> vmPoolTemp = new ArrayList<Vm>(vmPool);
		for(Vm vm: vmPoolTemp) {
			SDNVm svm = (SDNVm) vm;
			String type = svm.getMiddleboxType();
			if(type != null) {
				vnfPool.add(svm);
				vmPool.remove(svm);
			}
		...
		// policyTable.values()
		Collection<ServiceFunctionChainPolicy> sfcPolicies = this.sfcForwarder.getAllPolicies();
		for(ServiceFunctionChainPolicy sfcpolicy:sfcPolicies){
			...
			List<Integer> sfcVm = sfcpolicy.getServiceFunctionChainIncludeVM();
		}
		...
		//TODO Separate groups by SFC Policies
		VmGroup.classifyGroupbySFC(sfcPolicies, vnfPool, groups);
		
		// Sort VmGroup by their mips request
		// TODO sort VmGroup by their BW request
...
*/
	...
	@Override
	public boolean deployApplication(List<Vm> vms, List<Middlebox> middleboxes, List<Arc> links, List<ServiceFunctionChainPolicy> sfcPolicy) {
