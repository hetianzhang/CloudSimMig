...
public class VmGroup implements Comparable<VmGroup> {
	...
	protected static VmGroup findSfcGroup(List<VmGroup> groups, List<Integer> sfc) {
		for(VmGroup vmGroup:groups) {
			boolean flag = true;
			for(int i =0; i< vmGroup.getVms().size();i++) {
				if(vmGroup.getVms().get(i).getId() != sfc.get(i)) {
					flag = false;
					break;
				}
			}
	...
	public static void classifyGroupbySFC(Collection<ServiceFunctionChainPolicy> sfcPolicies, List<Vm> sfPool, List<VmGroup> groups) {
		for(ServiceFunctionChainPolicy sfcpolicy:sfcPolicies){
			//int srcId = sfcpolicy.getSrcId();
			//int dstId = sfcpolicy.getDstId();
			List<Integer> sfc = sfcpolicy.getServiceFunctionChain();
			//List<Integer> sfcWhole = sfcpolicy.getServiceFunctionChainIncludeVM();
			
			VmGroup vmGroup = null;
			
			vmGroup = findSfcGroup(groups, sfc);
			if(vmGroup == null) {
				//no sfc group exist
				vmGroup = new VmGroup();
				List<Vm> realSfc = new ArrayList<>();
				for(Integer n:sfc) {
					int index = VmGroup.findVm(sfPool, n);
					Vm vnf = sfPool.get(index);
					realSfc.add(vnf);
				}
				vmGroup.addVms(realSfc);
				groups.add(vmGroup);
			}
		}
	}
	
	public void addVms(List<Vm> sfc) {
		for(Vm sf: sfc) {
			int index = this.findVm(sf.getId());
			if(index == -1) {
				vms.add(sf);
				requiredBw += sf.getBw();
				requiredMips += ((SDNVm) sf).getTotalMips();
			}
