 ...
 */
public class PhysicalTopologyGeneratorSFC {
	...
	public static void startConst() {		
		String jsonFileName = "sfc.physical.fattree-k4-bw10-core96.json";
		//String jsonFileName = "paper.physical.fattree-k4-bw10-core48.json";
		//String jsonFileName = "planetlab.physical.fattree-k16-bw1-core48.json";
		//String jsonFileName = "depmig.physical.fattree-k8-core16.json";
		
//		int fanout = 2;
		int numPods = 4; 	// Total hosts = (numPods^3)/4 k=24,16,8 hosts=3456,1024,128
		double latency = 0.1;
		...
		long bw = 10000000000L; //1Gbps //10000000000L;//10Gbps //200000000 //500000000; //2000000000 2Gbps, 10000000000 10Gbps
...
//		reqg.createTreeTopology(hostSpec, iops, bw, fanout, latency);
		...
		reqg.createTopologyFatTreeHostNum(hostSpec, iops, bw, numPods, latency);
	...
	//TODO NEW dual-homing in the fat tree topology
	protected void createTopologyDualHomeFatTree(HostSpec hostSpec, long swIops, long swBw, int numpods, double latency) {
	...
	public void createPaperMotivationTopo(HostSpec hostSpec) {
		List<HostSpec> hostList = new ArrayList<>();
		for(int i =0; i<4; i++) {
			
		}
	}
	
	private static Integer totalHostNum = 0;
	
	protected void createTopologyFatTreeHostNum(HostSpec hostSpec, long swIops, long swBw, int numpods, double latency) {
		SwitchSpec [][] c = new SwitchSpec [numpods/2][numpods/2];
		for(int i=0; i<numpods/2; i++) {
			for(int j=0; j<numpods/2; j++) {
				c[i][j] = addSwitch("c_"+i+"_"+j, "core", swBw, swIops);
		...
		for(int k=0; k<numpods; k++) {
			...
			for(int i=0; i<numpods/2; i++) {
				...
				for(int j=0; j<numpods/2; j++) {
					String hostname = Integer.toString(totalHostNum);
					HostSpec h = addHost(hostname, hostSpec);
					totalHostNum += 1;
					addLink(e[i], h, latency);
				}
