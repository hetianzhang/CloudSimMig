...
public class PhysicalTopologyParser {
	...
	private Hashtable<Node, String> nodeNameTable = new Hashtable<Node, String>();
	...
	public Hashtable<Node, String> getNodeNameTable(){
		return nodeNameTable;
	}
	
	public Hashtable<String, Node> getNameNodeTable(){
		...
		try {
			...
			while(iter.hasNext()){
				...
				if(nodeType.equalsIgnoreCase("host")){
					...
					//BUG: long bw = new BigDecimal((Long)node.get("bw")).intValueExact();
					...
					for(int n = 0; n< num; n++) {
						...
						nodeNameTable.put(sdnHost, nodeName2);
						//hostId++;
			...
			while(linksIter.hasNext()){
				...
				Long upbw = (Long) link.get("upbw");
				Long downbw = (Long) link.get("downbw");
				
				Node srcNode = nameNodeTable.get(src);
				Node dstNode = nameNodeTable.get(dst);
				
				Link l = null;
				if(upbw == null || downbw == null || upbw ==0 || downbw == 0) {
					l = new Link(srcNode, dstNode, lat, -1);
				}else {
					l = new Link(srcNode, dstNode, lat, upbw, downbw);
				}
				this.links.add(l);
			}
